...
$(document).ready(function(){
    ...
    tb.off("click","#editItem2").on("click","#editItem2",function(){
        var tr=$(this).parent().parent().parent();  //改动html标签嵌套时，注意选择到tr
        tdData = new Array();
        tdData.push($(".carNumber",tr).text());
        tdData.push($(".color",tr).text());
        tdData.push($(".identity",tr).text());
        $(".carNumber",tr).html("<input type='text' value='"+$(".carNumber",tr).text()+"'>");
        $(".color",tr).html("<input type='text' value='"+$(".color",tr).text()+"'>");
        $(".identity",tr).html("<input disabled='disabled' type='text' value='"+$(".identity",tr).text()+"'>");
        $(".driver",tr).html("<select class='driverSelect'></select>");
        $(".vehicleType",tr).html("<select class='vehicleTypeSelect'></select>");
        showDV(tr);

        $(".driverSelect",tr).val($(".driverId",tr).text());
        $(".vehicleTypeSelect",tr).val($(".vehicleTypeId",tr).text());
        showEdit(tr);
    });
