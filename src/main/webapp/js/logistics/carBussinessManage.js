...
$(document).ready(function(){
    ...
    tb.off("click","#editItem2").on("click","#editItem2",function(){
        var tr=$(this).parent().parent().parent();  //改动html标签嵌套时，注意选择到tr
        tdData = new Array();
        tdData.push($(".startDate",tr).text());
        tdData.push($(".endDate",tr).text());
        $(".startDate",tr).html("<input id='startDateVal' type='date' value='"+$(".startDate",tr).text()+"'>");
        $(".endDate",tr).html("<input id='endDateVal' type='date' value='"+$(".endDate",tr).text()+"'>");
        $(".task",tr).html("<select class='taskSelect'></select>");
        $(".vehicle",tr).html("<select class='vehicleSelect'></select>");
        showDV(tr);
        $(".taskSelect",tr).val($(".taskId",tr).text());
    ...
    tb.off("click","#cancal").on("click","#cancal", function () {
        var tr=$(this).parent().parent().parent();  //改动html标签嵌套时，注意选择到tr
        $("input[type='date']",tr).each(function(index,element){
            element=$(element);
