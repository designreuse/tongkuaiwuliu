...
$(document).ready(function(){
    ...
    tb.off("click","#editItem2").on("click","#editItem2",function(){
        var tr=$(this).parent().parent().parent();  //改动html标签嵌套时，注意选择到tr
        tdData = new Array();
        tdData.push($(".litre",tr).text());
        tdData.push($(".price",tr).text());
        $(".litre",tr).html("<input type='text' value='"+$(".litre",tr).text()+"'>");
        $(".price",tr).html("<input type='text' value='"+$(".price",tr).text()+"'>");
        $(".staff",tr).html("<select class='staffSelect'></select>");
        $(".vehicle",tr).html("<select class='vehicleSelect'></select>");
        showDV(tr);
        $(".staffSelect",tr).val($(".staffId",tr).text());
        $(".vehicleSelect",tr).val($(".vehicleId",tr).text());
        showEdit(tr);
    });
    ...
    tb.off("click","#cancal").on("click","#cancal", function () {
        ...
        var vehicle=$(".vehicleSelect",tr).val();
        $(".staff",tr).empty();
        $(".vehicle",tr).empty();

        $(".staff",tr).append("<a href='/view/oilBuy/staffInfo.jsp?sid="+staff+"'>员工详情</a>");
        $(".vehicle",tr).append("<a href='/view/vehicle/vehicleInfo.jsp?sid="+vehicle+"'>车型详情</a>");
        showNormal(tr);
    });
